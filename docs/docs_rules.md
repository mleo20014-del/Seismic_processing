...
ADR1["ADR-001: python_everywhere"]
ADR2["ADR-002: agent_development"]
ADR_dots["..."]
end

subgraph  AGENT["AGENTS.md"]
AG1["ADR-001: Выбор архитектурного стека"]
AG2["ADR-002: Стратегия хранения данных"]
AG_dots["..."]
end
...
```
**Правило:** статьи о проектировании и разработке софта живут в репозитории рядом с кодом (*/docs*). Статьи по использованию софта (по модулям обработки) публикуются на сайте сообщества и индексируются отдельным RAG-контуром помощника внутри приложения.

Не смешивай эти два потока в одном файле или одной директории.

## 3. Структура файла статьи

Каждая статья — отдельный .md файл со следующей обязательной анатомией:
...
Сразу после заголовка YAML идёт абзац, отвечающий на вопрос «о чём эта статья» без необходимости читать её целиком. Это резко повышает качество поиска (retrieval quality) в системах RAG и знакомит человека с содержанием статьи.
...
> Используй *id*, а не название файла при создании ссылки — присвоенный статье *id* не должен меняться при переименовании файла.
...
или можно сделать связь через *id* в блоке "Связанные документы".
...
Пример:
...используется технология *RAG* (см. в [glossary.md](./glossary.md#архитектура-программного-обеспечения) внутри локальной ИИ-модели...

    ...используется технология [RAG](./glossary.md#архитектура-программного-обеспечения) внутри локальной ИИ-модели...

> Для создания такой ссылки необходимо установить формат "#### термин" в глоссарии, чтобы можно было создать ссылку на это слово/фразу. Затем необходимо скопировать путь из строки браузера и вставить как есть ссылку в контекст по примеру выше
...
• Не используй фразеологизмы, эпитеты, риторические и сложные речевые конструкции. Особенно при написании `[FACT]` фактов . Неформальный тон допустим только в Summary, TODO и в местах с пометкой `[ASSUMPTION]` перед текстом с мнением. Запрещено в разделах с `[FACT]``[DECISION]` утверждениями.
